...
(deftheme original-theme "original color theme")

(custom-theme-set-faces
 'original-theme
 '(cursor ((t (:background "brown"))))
 '(mouse ((t (:background "gainsboro"))))
 '(default ((t (:background "Grey10" :foreground "gainsboro"))))
 '(border ((t (:background "gainsboro"))))
 ...
 '(font-lock-builtin-face ((t (:foreground "rosy brown"))))
 ...
 '(font-lock-function-name-face ((t (:foreground "RosyBrown3"))))
 '(font-lock-keyword-face ((t (:foreground "#96CBFE"))))
 '(font-lock-reference-face ((t (:foreground "pale green"))))
 '(font-lock-string-face ((t (:foreground "PaleTurquoise3"))))
 '(font-lock-type-face ((t (:foreground "LemonChiffon2"))))
 ...
 ;;; Emacs23 では modeline でよかったが、Emacs24 では  mode-line という名前
 ;;; の face を使う。ただし、modeline の face を参照している elisp (skk?) も
 ;;; あるので modeline も残しておく
 '(mode-line ((t (:background "SteelBlue4" :foreground "white" :inverse-video nil :box (:line-width -1 :color "SteelBlue4" :style nil))))) ;; for Emacs24
 '(modeline ((t (:background "SteelBlue4" :foreground "white" :inverse-video nil :box (:line-width -1 :color "SteelBlue4" :style nil))))) ;; for Emacs23
 '(mode-line-inactive ((t (:inherit mode-line :background "gray30" :foreground "gray80"  :box (:line-width -1 :color "gray30" :style nil)))))
 ...
 '(my-paren-face ((t (:foreground "SkyBlue4"))))
 '(my-brace-face ((t (:foreground "SkyBlue4"))))
 '(my-bracket-face ((t (:foreground "SkyBlue4")))))

(defadvice enable-theme (after chg-var activate)
  (when (eq (ad-get-arg 0) 'original-theme)
    (setq display-time-mail-face 'mode-line)
    (setq gnus-article-button-face 'bold)
    (setq gnus-article-mouse-face  'highlight)
    (setq gnus-mouse-face 'highlight)
    (setq gnus-server-agent-face 'gnus-server-agent-face)
    (setq gnus-server-closed-face 'gnus-server-closed-face)
    (setq gnus-server-denied-face 'gnus-server-denied-face)
    (setq gnus-server-offline-face 'gnus-server-offline-face)
    (setq gnus-server-opened-face 'gnus-server-opened-face)
    (setq gnus-signature-face 'gnus-signature-face)
    (setq gnus-summary-selected-face 'gnus-summary-selected-face)
    (setq help-highlight-face 'underline)
    (setq list-matching-lines-face 'bold)
    (setq mime-button-face 'bold)
    (setq mime-button-mouse-face 'highlight)
    (setq sgml-set-face  t)
    (setq tags-tag-face 'default)
    (setq view-highlight-face 'highlight)
    (setq widget-mouse-face 'highlight)))

(enable-theme 'original-theme)
